      ...
      <div className="px-4 pt-3">
        {loading ? (
          ...
          <button key={c.id} onClick={() => navigate(`/chat/${c.id}`)} data-testid={`conversation-${c.id}`} className="w-full flex items-center gap-3 py-3.5 border-b border-white/5 text-left animate-fade-in">
            <Avatar className="w-12 h-12"><AvatarImage src={c.other_user?.picture} /><AvatarFallback className="bg-volt text-black">{c.other_user?.name?.[0]}</AvatarFallback></Avatar>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm">{c.other_user?.name}</p>
              <p className="text-xs text-zinc-500 truncate">{c.last_message || "Say hi 👋"}</p>
            </div>
            {c.unread_count > 0 && (
